...
const seashell_imxl = 641;
const seashell_imxl2 = seashell_imxl / 2;
const seashell_ianzmax = 9; 
const seashell_msiluetmax = 500;
...
class SeaShells {
    ...
    transition(position){
     ...
     initial_state() {
      ...
      imxnew = seashell_imxl2;
      if (ky > imxnew) 
         {imxnew = seashell_imxl}
      ...
      a[0, i] = ra * (1.0 + 2 * kr / 100 * (Math.random(1) - .5)) // 'source density s
      a[8, i] = 1
      a[9, i] = 1 + 2 * kr / 100 * (Math.random(1) - .5)
      if (ca > 0) {a[0, i] = ca * (1 + 2 * kr / 100 * (Math.random(1) - .5))}
      }
  ...
  case "i7": //sinusoidal prepattern, used for space-dependent
    ...
    for (i = 1; i <=  ky; i++)
    {
      a[8, i] = 1 + dy * (Math.cos((i - k2) * 3.14 / K1)) ^ 2 + i * ab / ky
  ...
  case "i8": // Exponential gradient in the source density
    for( ix = 1; ix  <=  ky; ix ++) {  a[0, ix] = a[0, ix]* Math.exp(-ab * (ix - 1));} 
    break;
